class _loader
{
	...
	load( _elemId )
	{
		...
		console.log( 'loader loading ' + _elemId );
		...
		return new Promise(
			( _success, _fail ) =>
			{
				try
				{
					...
					if( 'undefined' != typeof _replaces && _replaces.length )
					{
						for( let _i in _replaces )
						{
							...
							console.log( 'replace ' + _replace + ' with ' + $( _replace ).val() );
						...
						( _ret ) =>
						{
							...
							if( 1 == _ret.return )
							{
								...
								else
								{
									...
									if( _emptyMsg )
									{
										console.log( 'emptyMsg ' + _emptyMsg );
							...
							if( _emptyMsg )
							{
								console.log( 'emptyMsg failed-return ' + _emptyMsg );
								_popTarget.children().not( '.keep' ).remove();
								_popTarget.append( _emptyMsg );
							}
				...
				{
					console.log( '_loader load error' );
					console.log( _error );
					return _fail( _error );
				}
			}
