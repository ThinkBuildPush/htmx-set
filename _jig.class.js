class _jig
{
				...
				$( '.autotpl' ).each(
					( _index, _elem ) =>
					{
						...
						.catch(
							( _msg ) =>
							{
								console.log( _id + ' failed autotpl with message ' + _msg );
							}
						);
					}
				);
	...
	postPop( _id )
	{
		...
		console.log( 'postPop settoggle' );
		console.log( ('#' + _id).replace( '##', '#' ) );
		$( ('#' + _id).replace( '##', '#' ) ).find( '.autotoggle' ).each(
			( _toggleIndex, _toggleElem ) =>
			{
				console.log( 'toggling' );
				console.log( _toggleElem );
	...
	popTpl()
	{
		...
		{
			console.log( _tplHandle + ' not found' );
